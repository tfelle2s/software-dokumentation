...
public class GoogleBigQueryController {
    ...
    private GoogleBigQueryController() throws IOException {
        String key = "key.json";
        this.bigQuery = BigQueryOptions.newBuilder().setProjectId(Credentials.PROJECT_ID)
                .setCredentials(
    ...
    public ArrayList<SoAnswersWithUserNameDTO> getAnswersWithUserNameForRefLink(String refLink) throws NoValidStackOverflowLink, GoogleBigQueryException, InterruptedException {

        String query = QueryBuilderFactory.createGetAllAnswersAndUsersForQuestionQuery(refLink);
        TableResult result = runQueryJob(query);

        ArrayList<SoAnswersWithUserNameDTO> possibleAnswers = new ArrayList<>();
        for (FieldValueList row : result.iterateAll()) {
            SoAnswersWithUserNameDTO answer = new SoAnswersWithUserNameDTO();
    ...
    public String getDecisionByFileAndRepoName(String path, String repo_name) throws GoogleBigQueryException, InterruptedException {
        String query = QueryBuilderFactory.createQueryToGetGithubDecisionBodyByPathAndRepoName(path, repo_name);
        TableResult result = runQueryJob(query);
        for (FieldValueList row: result.iterateAll()) {
            return "Id: "+ row.get("id").getStringValue() + " Content: "+ row.get("content").getStringValue();
        }
        return null;
    }

    private TableResult runQueryJob(String query) throws GoogleBigQueryException, InterruptedException {

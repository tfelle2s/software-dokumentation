...
import org.arc42.dokumentation.control.service.exception.NoValidStackOverflowLink;
import org.arc42.dokumentation.model.dto.documentation.SoAnswersWithUserNameDTO;
...
public class DesignDecisionParser {
    ...
    public static List<UMLComponent> parseFile(String file) {
        ...
        return l.stream().filter(x -> (x instanceof ExistenceDecision)).map(decision -> {
            ExistenceDecision decision1 = (ExistenceDecision) decision;
            try {
                DDRationale rationale = ((ExistenceDecision) decision).getRationale();
                if (rationale != null && rationale.getReferenz() != null && !rationale.getReferenz().isEmpty()) {
                    ...
                    if (reference.trim().toLowerCase().contains("stackoverflow".toLowerCase())) {
                        ArrayList<SoAnswersWithUserNameDTO> answersWithUserNameForRefLink = controller.getAnswersWithUserNameForRefLink(reference);
                        ...
                        if (reference.trim().toLowerCase().contains("raw.githubusercontent")) {
                            ...
                            String[] repoNameAndFileFromGithubLink = StaticUtils.getRepoNameAndFileFromGithubLink(reference);
                            String mdContent = controller.getDecisionByFileAndRepoName(repoNameAndFileFromGithubLink[1], repoNameAndFileFromGithubLink[0]);
                            decision1 = parseGitHubContent(mdContent, decision1);
                        }
                    }
                }
            } catch (IOException | InterruptedException | GoogleBigQueryException | NoValidStackOverflowLink e) {
                e.printStackTrace();
            }
    ...
    private static ExistenceDecision parseStackoverflowContent(ArrayList<SoAnswersWithUserNameDTO> answersForRefLink, ExistenceDecision decision) {
        Set<DDAlternative> alternatives = new HashSet<>(10);
        String choice = decision.getBeschreibung();
        answersForRefLink.forEach(soAnswersDTO -> {
            System.out.println(soAnswersDTO.getUserId() + "---------" + choice + "-------UNAME");
            DDAlternative alternative = new DDAlternative(soAnswersDTO.getAnswerId(), soAnswersDTO.getAnswerBody(), soAnswersDTO.getAnswerBody());
    ...
    private static ExistenceDecision parseGitHubContent(String mdContent, ExistenceDecision decision) {
        String choice = decision.getBeschreibung();
        ...
        for (String content : contents) {
            if (content.toLowerCase().contains("Considered Options".toLowerCase()) || content.toLowerCase().contains("Considered Alternatives".toLowerCase())) {
                ...
                for (int i = 0; i < alternatives.length; i++) {
                    ...
                    DDAlternative ddAlternative = new DDAlternative(UUID.randomUUID().toString(), "", render.trim());
            ...
            if (content.toLowerCase().contains("Pros and Cons".toLowerCase())) {
                ...
                for (int j = 0; j < vornach_teilen.length; j++) {
                    ...
                    for (int i = 0; i < split.length; i++) {
                        ...
                        if (s.toLowerCase().replaceFirst("\\*", "").trim().startsWith("Good".toLowerCase()) || s.toLowerCase().replaceFirst("\\*", "").trim().startsWith("`+`".toLowerCase())) {
                        ...
                        if (s.toLowerCase().replaceFirst("\\*", "").trim().startsWith("Bad".toLowerCase()) || s.toLowerCase().replaceFirst("\\*", "").trim().startsWith("`-`".toLowerCase())) {
                            org.commonmark.parser.Parser parser = org.commonmark.parser.Parser.builder().build();
                            Node document = parser.parse(s);
                            HtmlRenderer renderer = HtmlRenderer.builder().build();
                            String render = renderer.render(document);

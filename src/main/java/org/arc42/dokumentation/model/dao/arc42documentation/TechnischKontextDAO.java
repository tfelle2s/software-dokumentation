            ...
            if (arcId != null) {
                return session.writeTransaction(transaction -> {
                    ...
                    Result result = transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    " create (a)-[r:kontext]->(s:Kontext:TechnischhKontext {tkontext:$tkontext})" + System.lineSeparator() +
            ...
            if (arcId != null) {
                session.writeTransaction(transaction -> {
                    transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    "match (a)-[r:kontext]->(s:Kontext:TechnischhKontext) detach delete s return count(*)",
                            parameters("id", arcId));
                    return 0;
                });
            }
    ...
    public ImageDTO findById(String id) {
        try (Session session = getDriver().session()){
            ...
            if (arcId != null) {
                return session.writeTransaction(transaction -> {
                    Result result = transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    " match (a)-[r:kontext]->(s:Kontext:TechnischhKontext)" + System.lineSeparator() +
                                    "return Id(s), s.tkontext",
                            parameters("id", arcId));
                    ImageDTO dto = null;
                    if (result != null) {

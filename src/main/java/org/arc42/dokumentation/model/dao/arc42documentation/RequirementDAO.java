...
public class RequirementDAO extends ARC42DAOAbstract<RequirementDTO, String> {
    ...
    public RequirementDTO save(RequirementDTO requirementDTO) {
        ...
        if (arcId != null) {
            try (Session session = getDriver().session()){
                String ausgabe = session.writeTransaction(transaction -> {
                    Result result = transaction.run("match (d:Arc42) where Id(d)=$arc" + System.lineSeparator() +
                            "create (d)-[r:hasRequirement]->(a:Aufgabenstellung {aufgabe: $message}) return a.aufgabe, Id(a)", parameters("arc", arcId, "message", requirementDTO.getAufgabe()));
                    Record record = result.next();
                    return record.get("a.aufgabe") + ":" + record.get("Id(a)");
                });
    ...
    public void update(RequirementDTO requirementDTO) {
        try (Session session = getDriver().session()){
            session.writeTransaction(transaction -> {
                Result result = transaction.run("match(a:Aufgabenstellung) where Id(a)=$id set a.aufgabe=$neu return a.aufgabe, Id(a)", parameters("id", Integer.parseInt(requirementDTO.getId()), "neu", requirementDTO.getAufgabe()));
                RequirementDTO dto = null;
                if (result != null) {
                    Record record = result.single();
                    dto = new RequirementDTO(record.get("a.aufgabe").asString());
    ...
    public Boolean delete(RequirementDTO requirementDTO) {
        try (Session session = getDriver().session()){
            return session.writeTransaction(transaction -> {
                Result result = transaction.run("match(a:Aufgabenstellung)where Id(a)=$id" + System.lineSeparator() +
                        "match(d:Arc42) match (a)<-[r:hasRequirement]-(d) delete r,a return count(*)", parameters("id", Integer.parseInt(requirementDTO.getId())));
                Record record = result.single();
    ...
    public List<RequirementDTO> findAllByArcId(String id) {
        try(Session session = getDriver().session()) {
            return session.writeTransaction(transaction -> {
                Result result = transaction.run("match(d:Arc42) where Id(d)=$id match(a:Aufgabenstellung) match (d)-[r:hasRequirement]->(a) return a.aufgabe, Id(a)", parameters("id", Integer.parseInt(id)));
    ...
    public RequirementDTO findById(String id) {
        if (id != null && !id.isEmpty()) {
            ...
            try (Session session = getDriver().session()) {
                return session.writeTransaction(transaction -> {
                    Result result = transaction.run("match(a:Aufgabenstellung) where Id(a)=$id return Id(a), a.aufgabe", parameters("id", arcId));

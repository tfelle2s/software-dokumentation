            ...
            if (arcId != null) {
                return session.writeTransaction(transaction -> {
                    ...
                    Result result = transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    " create (a)-[r:laufzeitsicht]->(s:Image:Laufzeit {bildName:$bildName, bildMimeType:$bildMimeType, bildPath:$bildPath, uxfName:$uxfName, uxfMimeType:$uxfMimeType, uxfPath:$uxfPath, description:$description})" + System.lineSeparator() +
            ...
            if (arcId != null) {
                session.writeTransaction(transaction -> {
                    transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    "match (a)-[r:laufzeitsicht]->(s:Image:Laufzeit) detach delete s return count(*)",
                            parameters("id", arcId));
                    return 0;
                });
            }
    ...
    public ImageDTO findById(String id) {
        try (Session session = getDriver().session()){
            ...
            if (arcId != null) {
                return session.writeTransaction(transaction -> {
                    Result result = transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    " match (a)-[r:laufzeitsicht]->(s:Image:Laufzeit)" + System.lineSeparator() +
                                    "return Id(s), s.bildName, s.bildMimeType, s.bildPath, s.uxfName, s.uxfMimeType, s.uxfPath, s.description",
                            parameters("id", arcId));
                    ImageDTO dto = null;
                    if (result != null) {

...
public class QualityCriteriaComponent extends HorizontalLayout {
    ...
    public QualityCriteriaComponent(Grid qualityG, ARC42DAOAbstract dao) {
        ...
        HorizontalLayout interactionLayout = new HorizontalLayout(verticalLayout, selectedLayout, notSelectedLayout);
    ...
    private void createListener(BadgeComponent badgeComponent) {
        badgeComponent.addClickListener((ComponentEventListener<ClickEvent<Span>>) event -> {
            if (badgeComponent.getParent().get() == notSelectedBatch && badgeComponent.getParent().get().getChildren().count() > 6) {
                ...
                new NotificationWindow("Sie können nur maximal zwei Kategorien pro Ziel auswählen!", NotificationType.SHORT, NotificationType.ERROR);
            }
        });
    }

    List getCategories(VerticalLayout component) {
    ...
    public void addButtonListener(QualityDTO qualityDTO) {
        buttonComponent.getEdit().addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if (getCategories(selectedBatch).size() != 0) {
                ...
                textFieldComponent.clear();
                updateList();
                new NotificationWindow("Qualitätsziel erfolgreich geändert!", NotificationType.SHORT, NotificationType.SUCCESS);
            } else {
                new NotificationWindow("Sie müssen mindestens einen Kategorie auswählen!", NotificationType.MEDIUM, NotificationType.ERROR);
            }
        });

        buttonComponent.getDelete().addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            this.dao.delete(qualityDTO);
            textFieldComponent.clear();
            updateList();
            new NotificationWindow("Qualitätsziel erfolgreich gelöscht!", NotificationType.SHORT, NotificationType.NEUTRAL);
        });
    }

    public void addCreateListener() {
        buttonComponent.getCreate().addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if(!textFieldComponent.isEmpty()) {
                List list = getCategories(selectedBatch);
                if (list.size() != 0) {
                    if (dao instanceof QualityGoalDAO) {
                        qualityDTO = new QualityGoalDTO(textFieldComponent.getTextField1().getValue(), textFieldComponent.getTextField2().getValue(), getCategories(selectedBatch));
                    } else {
                        qualityDTO = new QualityScenarioDTO(textFieldComponent.getTextField1().getValue(), textFieldComponent.getTextField2().getValue(), getCategories(selectedBatch));
                    }
                ...
                new NotificationWindow("Sie müssen beide Textfelder füllen!", NotificationType.MEDIUM, NotificationType.ERROR);
            }
        });

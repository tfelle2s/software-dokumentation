...
import org.arc42.dokumentation.control.util.DesignDecisionParser;
...
public class UploadComponent extends VerticalLayout {
    ...
    public UploadComponent(ARC42DAOAbstract dao,String url) {
        ...
        i18n.getError().setIncorrectFileType("Die Datei hat das falsche Format. Bitte ein Bild mit dem Format .png hochladen");
        ...
        i18n1.getError().setIncorrectFileType("Die Datei hat das falsche Format. Bitte eine Datei mit dem Format .uxf hochladen");
        ...
        if (dao.getActualArcId(url) != null) {
            ...
            if (imageDTO != null) {
                if (this.imageDTO.showImage()) {
                    ...
                    showOutput(imageDTO.getBildMimeType(),
                            createComponent(imageDTO.getBildMimeType(),imageDTO.getBildName(),targetStream)
                    );
                    text = new Label("Aktuell wird folgende .uxf Datei verwendet: " + imageDTO.getUxfName());
                    uxfLayout.add(text);
                }
                } else {
                text = new Label("Noch nichts hochgeladen");
                uxfLayout.add(text);
            }
        }
        ...
        uploadImage.addSucceededListener(event -> {
            ...
            Component component = createComponent(event.getMIMEType(),
                    event.getFileName(), bufferImage.getInputStream());
            showOutput(event.getFileName(), component);
            new NotificationWindow("Sie haben die Bild-Datei erfolgreich hochgeladen", NotificationType.MEDIUM,NotificationType.SUCCESS);

        });

        uploadUxf.addSucceededListener(event -> {
            if(imageDTO==null) {
                imageDTO = new ImageDTO();
            ...
            text = new Label("Sie haben folgende .uxf Datei erfolgreich hochgeladen: " + event.getFileName());
            uxfLayout.add(text);
            new NotificationWindow("Sie haben die .uxf-Datei erfolgreich hochgeladen!",5000,"success");
        });
        ...
        save.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if (imageDTO != null && imageDTO.getBildName() != null && imageDTO.getUxfName() != null) {
                imageDTO = (ImageDTO) dao.save(imageDTO);
                File file = new File("src/main/resources/uploads/test.uxf");
                try {
                    FileUtils.copyInputStreamToFile(new ByteArrayInputStream(imageDTO.getUxfStream()), file);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                DesignDecisionDAO dao1 = DesignDecisionDAO.getInstance();
                List<UMLComponent> decisions = DesignDecisionParser.parseFile(file.getAbsolutePath());
                decisions.stream().filter(x -> x instanceof ExistenceDecision).forEach(x -> dao1.save(imageDTO, (ExistenceDecision) x));
                new NotificationWindow("Erfolgreich gespeichert!", 5000, "success");
                clearFileList();
                file.deleteOnExit();
            } else {
                new NotificationWindow("Bitte laden Sie beide Dateien hoch bevor Sie speichern!", 5000, "error");
            }
        });

        cancel.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if (imageDTO != null) {
                ...
                new NotificationWindow("Keine Daten vorhanden! Es wurde nichts gelöscht.", 5000, "neutral");
            }
        });

...
public class ARC42Konzepte extends Arc42NewDocumentationView{
    ...
    public void init()  {
        super.init();
        getKonzepte().getSummary().getElement().getStyle().set(ElementConstants.STYLE_COLOR, "var(--lumo-primary-text-color)");
        getKonzepte().setOpened(true);

        ARC42DAOAbstract dao = KonzepteDAO.getInstance();
        VerticalLayout verticalLayout = new VerticalLayout();
        verticalLayout.setSizeFull();
        ImageDTO imageDTO = (ImageDTO) dao.findById(url);


        TextAreaComponent textAreaComponent = new TextAreaComponent("Konzepte", imageDTO.getDescription(), dao);
        ...
        verticalLayout.add(new BreadCrumbComponent(new String[]{"Konzepte"}),textAreaComponent);
        add(verticalLayout);
    }
}
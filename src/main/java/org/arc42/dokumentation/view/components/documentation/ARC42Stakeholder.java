    ...
    public void init() {
        ...
        addStakeholder.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if(!(rolle.getValue().isEmpty() || kontakt.getValue().isEmpty() || erwartung.getValue().isEmpty())) {
                ...
                new NotificationWindow("Es müssen alle Felder ausgefüllt werden!", NotificationType.SHORT, NotificationType.NEUTRAL);
            }
        });
        ...
        edit.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            ...
            clearTextFieldValue();
            updateList();
            new NotificationWindow("Stakeholder erfolgreich geändert!", NotificationType.SHORT, NotificationType.SUCCESS);
        });
        ...
        delete.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            ...
            clearTextFieldValue();
            updateList();
            new NotificationWindow("Stakeholder erfolgreich gelöscht!", NotificationType.SHORT, NotificationType.NEUTRAL);

        });

        VerticalLayout main = new VerticalLayout();
        main.setHorizontalComponentAlignment(Alignment.STRETCH, gridLayout);
        main.add(new BreadCrumbComponent(new String[]{"Einführung und Ziele", "Stakeholder"}), gridLayout, horizontalLayout, buttonLayout);
        add(main);
    }

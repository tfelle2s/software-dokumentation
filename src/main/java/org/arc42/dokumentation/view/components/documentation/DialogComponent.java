...
public class DialogComponent extends Div {
    ...
    public DialogComponent(String dialog) {
        switch (dialog) {
            case "delete":  this.dialog = createDialogDelete(this.dialog);
                break;
            case "createDoku": this.dialog = createDialogCreateDoku();
                break;
            case "notSave": this.dialog = createNotSavedDialog();
        }
    ...
    private Dialog createNotSavedDialog() {
        ...
        H4 body = new H4("Sie haben Änderungen vorgenommen aber nicht gespeichert. Möchten Sie wirklich fortfahren?");
        ...
        HorizontalLayout buttonLayout = new HorizontalLayout( this.deleteButton,
                this.createButton);
        buttonLayout.setJustifyContentMode(FlexComponent.JustifyContentMode.END);
        buttonLayout.getStyle().set("margin-top", "var(--lumo-space-m)");

        VerticalLayout dialogLayout = new VerticalLayout(headline, body,
                buttonLayout);
    ...
    private Dialog createDialogCreateDoku() {
        ...
        VerticalLayout dialogLayout = new VerticalLayout(headline, this.titel,
                buttonLayout);
        dialogLayout.setPadding(false);
        dialogLayout.setSpacing(false);
        dialogLayout.setAlignItems(FlexComponent.Alignment.STRETCH);

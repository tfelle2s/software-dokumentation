...
public class ARC42Organisatorisch extends Arc42NewDocumentationView{
    ...
    public void init() {
        ...
        getOrganisatorisch().getStyle().set(ElementConstants.STYLE_COLOR, "var(--lumo-primary-text-color)");
        ...
        addRandbedingung.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if (!(randbedingung.getValue().isEmpty() || erlaueterung.getValue().isEmpty())) {
                OrganisatorischDTO organisatorischDTO = new OrganisatorischDTO(randbedingung.getValue(), erlaueterung.getValue());
                dao.save(organisatorischDTO);
                clearTextFieldValue();
                updateList();
            } else {
                new NotificationWindow("Es müssen alle Eingabefelder ausgefüllt werden!", NotificationType.SHORT, NotificationType.NEUTRAL);
            }
        });
        ...
        orgG.addSelectionListener((SelectionListener<Grid<OrganisatorischDTO>, OrganisatorischDTO>) selection -> {
        ...
        edit.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            ...
            clearTextFieldValue();
            updateList();
            new NotificationWindow("Organisatorische Randbedingung erfolgreich geändert!",NotificationType.SHORT,NotificationType.SUCCESS);
        });
        ...
        delete.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            ...
            clearTextFieldValue();
            updateList();
            new NotificationWindow("Organisatorische Randbedingung erfolgreich gelöscht!",NotificationType.SHORT,NotificationType.NEUTRAL);
        });

        VerticalLayout main =  new VerticalLayout();
        main.setHorizontalComponentAlignment(Alignment.STRETCH,gridLayout);
        main.add(new BreadCrumbComponent(new String[]{"Randbedingungen","Organisatorisch"}),gridLayout,horizontalLayout,buttonLayout);
        add(main);
    }

...
public class ARC42QualitySzenario extends Arc42NewDocumentationView{


    private VerticalLayout notSelectedLayout;
    private Grid<QualityGoalDTO> qualityGoalG;
    private Grid<QualityScenarioDTO> qualityScenarioG;
    ...
    public void init() {
        ...
        getQualitaetsszenario().getStyle().set(ElementConstants.STYLE_COLOR, "var(--lumo-primary-text-color)");

        H4 header = new H4(new Text("Folgende Qualitätsziele wurden in: "), new BreadCrumbComponent(new String[]{"Einführung und Ziele","Qualitätsziele"}), new Text( "definiert:"));
        ...
        HorizontalLayout horizontalLayout1 = new HorizontalLayout(scenarioName,stimulus,reaction,response);
        ...
        binder.forField(risk)
                .bind(QualityScenarioDTO::getRisk,QualityScenarioDTO::setRisk);
        binder.forField(selectQualityGoal)
        ...
        this.qualityScenarioG = new Grid(QualityScenarioDTO.class,false);
        qualityCriteria =  new QualityCriteriaComponent(qualityScenarioG,dao);
        qualityCriteria.addCreateListener();


        qualityScenarioG.addColumn(QualityScenarioDTO::getId).setHeader("Id").setFlexGrow(0).setSortable(true);
        qualityScenarioG.addColumn(qualityScenarioDTO1 ->
                qualityScenarioDTO1.getQualityGoalDTO()!=null ? qualityScenarioDTO1.getQualityGoalDTO().getId() : null).setHeader("Zugehöriges Qualitätsziel").setFlexGrow(0).setResizable(true);
        qualityScenarioG.addColumn(new ComponentRenderer<>(QualityScenarioDTO::generateBadge)).setHeader("Kategorie").setAutoWidth(true).setResizable(true);
        qualityScenarioG.addColumn(QualityScenarioDTO::getScenarioName).setHeader("Qualitätsszenario").setResizable(true);
        qualityScenarioG.addColumn(QualityScenarioDTO::getStimulus).setHeader("Stimulus").setResizable(true);
        qualityScenarioG.addColumn(QualityScenarioDTO::getReaction).setHeader("Reaktion").setResizable(true);
        qualityScenarioG.addColumn(QualityScenarioDTO::getResponse).setHeader("Zielwert").setResizable(true);
        qualityScenarioG.addColumn(QualityScenarioDTO::getCurrentPriority).setHeader("Priorität").setFlexGrow(0).setAutoWidth(true);
        qualityScenarioG.addColumn(QualityScenarioDTO::getRisk).setHeader("Risiko").setFlexGrow(0).setResizable(true);
        ...
        HorizontalLayout horizontalLayout = new HorizontalLayout(selectQualityGoal,priority,risk,qualityCriteria);

        qualityScenarioG.addThemeVariants(GridVariant.LUMO_ROW_STRIPES);

        selectQualityGoal.addValueChangeListener((HasValue.ValueChangeListener<AbstractField.ComponentValueChangeEvent<ComboBox<QualityDTO>, QualityDTO>>) event -> qualityCriteria.setVisible(event.getValue() == null));

        create.addClickListener((ComponentEventListener<ClickEvent<Button>>) event -> {
            if (qualityCriteria.getSelectedBatch().getComponentCount() == 0 && qualityCriteria.isVisible()) {
                new NotificationWindow("Sie müssen mindestens einen Kategorie auswählen!", 4000, "error");
            } else {
                QualityScenarioDTO qualityScenarioDTO = new QualityScenarioDTO("", "",
                        qualityCriteria.isVisible() ? qualityCriteria.getCategories(qualityCriteria.getSelectedBatch()) : null);
       ...
       // update = new Button("Ändern");
        ...
        qualityScenarioG.addSelectionListener((SelectionListener<Grid<QualityScenarioDTO>, QualityScenarioDTO>) selectionEvent -> {
        ...
        VerticalLayout verticalLayout1 = new VerticalLayout(horizontalLayout1,horizontalLayout,create,delete);
        HorizontalLayout horizontalLayout2 = new HorizontalLayout(verticalLayout1,qualityCriteria);


        verticalLayout.add(new BreadCrumbComponent(new String[]{"Qualitätsszenarien","Szenarien"}),header, qualityGoalG,qualityScenarioG,horizontalLayout2);

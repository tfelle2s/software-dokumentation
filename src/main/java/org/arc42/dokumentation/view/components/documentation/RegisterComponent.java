...
public class RegisterComponent extends Div {
    ...
    private VerticalLayout createDialogLayout(Dialog dialog) {
        ...
        H5 body = new H5("Bitte geben Sie für die Registrierung einen Benutzernamen und ein Passwort ein!");
        ...
        username.addValueChangeListener((HasValue.ValueChangeListener<AbstractField.ComponentValueChangeEvent<TextField, String>>) textFieldStringComponentValueChangeEvent -> username.setInvalid(false));
        ...
        VerticalLayout dialogLayout = new VerticalLayout(headline, body,username,password,
                buttonLayout);
        ...
        Binder<FE_LoginDTO> binder = new Binder<>(FE_LoginDTO.class);
        binder.forField(username)
                .asRequired("Benutzername muss angegeben werden!")
                .bind(FE_LoginDTO::getUsername,FE_LoginDTO::setUsername);
        binder.forField(password)
                .asRequired("Bitte Passwort eingeben!")
                .withValidator(new StringLengthValidator(
                        "Bitte mindestens 8 Zeichen!", 8, null))
                .bind(FE_LoginDTO::getPassword,FE_LoginDTO::setPassword);
        binder.addStatusChangeListener(
                event -> binder.isValid());

        registerButton.addClickListener((ComponentEventListener<ClickEvent<Button>>) buttonClickEvent -> {
            if (binder.validate().isOk()) {
                FE_LoginDTO logindto = new FE_LoginDTO(username.getValue(), password.getValue());
                try {
                    if (Login.userExist(logindto)) {
                        username.setInvalid(true);
                        username.setErrorMessage("Benutzer existiert bereits! Bitte einen anderen wählen.");
                    } else {
                        Login.register(logindto);
                        dialog.close();
                        Notification notification = new Notification("Benutzer " + username.getValue() + " erfolgreich erstellt!");

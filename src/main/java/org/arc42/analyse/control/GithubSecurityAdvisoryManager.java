...
public class GithubSecurityAdvisoryManager {
    ...
    private void readGitHubSecurityAdvisory(String url) {
        try {
            ...
            List<GitHubSecurityAdvisoryDTO> feeds = entries.stream().filter(entry -> checkCategories((List<SyndCategoryImpl>) entry.getCategories())).map(entry -> new GitHubSecurityAdvisoryDTO(entry.getUri(), entry.getPublishedDate(), entry.getUpdatedDate(), entry.getTitle(), "MAVEN", ((SyndContentImpl) entry.getContents().get(0)).getValue())).collect(Collectors.toList());
            if (feeds.size() >= 1) {
...
//        return this.service.findAllGitHubSecurityAdvisory();
//    }

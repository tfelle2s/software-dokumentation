        ...
        if (count >= 1) {
            ...
            for (DesignDecisionDTOForSA decision : decisions) {
                for (GitHubSecurityAdvisoryDTO securityAdvisory : securityAdvisories) {
                    if (securityAdvisory.getTitle().trim().toLowerCase().contains(decision.getTitle().trim().toLowerCase())) {
                        System.out.println("Hinweis vielleicht Sicherheitslücke");
                        String li = "<li>" + decision.getTitle() + " :\n" + "<div >\n" + "  <p style=\"background-color: #ffff00;\">\n" + "    <em>hat möglicherweise Sicherheitslücken. Sie sollten die untenstehenden Sicherheitslücke-Beschreibung gut lesen und bewerten, ob Ihre Software gefährdet ist.</em></p>\n" + "<p><em>betroffene Softwareartefakt(e) : " + this.service.getAffectedArchitekturElement(decision.getId()) + "</em></p>\n" + "</div>\n" + "<p><em><strong>Sicherheitslücke-Beschreibung:</strong></em></p>\n" + "<p>veröffentlicht am : " + securityAdvisory.getPublished() + "</p>\n" + "<div style=\"color: #ff0000;\">" + securityAdvisory.getContent() + "</div>\n" + "</li>";
                        output.append(li);
                    }
                }
            }
            ...
            output.append("<p><strong><span style=\"color: #3BD700;\">&nbsp; &nbsp;- Es wurden keine Sicherheitslücken gefunden!</span></strong></p> </ol>");
        }

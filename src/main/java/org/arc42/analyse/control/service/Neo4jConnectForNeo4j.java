...
public class Neo4jConnectForNeo4j implements AutoCloseable {
    ...
    public String getTitleByArcId(Integer arcId) {
        ...
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id return a.name", parameters("id", arcId));
            if (result != null) {
    ...
    public int countDocumentedElemente(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r]-(b) return count(b)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countAufgabenstellung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match (d:Arc42) where Id(d)=$id match (d)-[r:hasRequirement]->(a:Aufgabenstellung) return count(a)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countQualitaetsZiele(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run(" match (d:Arc42) where Id(d)=$id match (d)-[r:hasQualityGoal]->(a:Qualitaetsziel) return count(a)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(a)").asInt();
            }
    ...
    public int countStakeholder(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (s:Stakeholder)<-[r:hasStakeholder]-(a) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countTechnischRandbedingung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match (d:Arc42) where Id(d)=$id match(d)-[r:hatTechnischRandbedingung]->(a:TechnischeRandbedingung) return count(a)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(a)").asInt();
            }
    ...
    public int countOrganisatorischRandbedingung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match (d:Arc42) where Id(d)=$id match (d)-[r:hatOrganisatorischRandbedingung]->(a:OrganisatorischRandbedingung) return count(a)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(a)").asInt();
            }
    ...
    public int countKonventionRandbedingung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match (d:Arc42) where Id(d)=$id match (d)-[r:hatKonvention]->(a:Konvention) return count(a)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(a)").asInt();
            }
    ...
    public int countFachlicherKontext(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:kontext]->(s:Kontext:FachlichKontext) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getFachlicherKontext(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:kontext]->(s:Kontext:FachlichKontext) return s.fkontext", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countTechnischerKontext(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("    match(a:Arc42) where Id(a)=$id match (a)-[r:kontext]->(s:Kontext:TechnischhKontext) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getTechnischerKontext(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:kontext]->(s:Kontext:TechnischhKontext) return s.tkontext", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countLoesungsStrategie(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatLoesung]->(s:LoesungStrategie) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getLoesungsStrategie(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatLoesung]->(s:LoesungStrategie) return s.loesung", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countBausteinDiagram(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:bausteinsicht]->(s:Image:Baustein) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getBausteinBeschreibung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:bausteinsicht]->(s:Image:Baustein) return s.description", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countLaufzeitDiagram(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:laufzeitsicht]->(s:Image:Laufzeit) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getLaufzeitBeschreibung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:laufzeitsicht]->(s:Image:Laufzeit) return s.description", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("s.description").asString();
            }
    ...
    public int countVerteilungDiagram(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:verteilungssicht]->(s:Image:Verteilung) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getVerteilungBeschreibung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:verteilungssicht]->(s:Image:Verteilung) return s.description", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("s.description").asString();
            }
    ...
    public int countKonzepte(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatKonzepte]->(s:Konzepte) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getKonzepte(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatKonzepte]->(s:Konzepte) return s.konzepte", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countArchitekturEntscheidung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatEntscheidung]->(s:EntwurfsEntscheidung) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getArchitekturEntscheidung(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatEntscheidung]->(s:EntwurfsEntscheidung) return s.entscheidung", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countDesignDecision(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision) return count(v)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public List<DesignDecisionDTOForSA> findDesignDecisionForArcId(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision) return v.id, v.title", parameters("id", arcId));
            if (result != null) {
                return new ArrayList<>(result.list(single -> new DesignDecisionDTOForSA(single.get("v.id").asString(), single.get("v.title").asString())));
            }
            return null;
        });
    }
    ...
    public String getDecisionIdForRationaleId(Integer arcId, String rationaleId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) match(z:Rationale) where Id(a)=$id and z.id=$rationaleId match (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision)-[c:wirdBegründetDurch]->(z) return v.id", parameters("id", arcId, "rationaleId", rationaleId));
            if (result != null) {
    ...
    public String getReferencedAlternativeId(String ddId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(d:DesignDecision) where d.id=$id match (d)-[a:ausgewaehlt]->(c:DDAlternative) return c.id", parameters("id", ddId));
            if (result != null) {
    ...
    public String getReferencedAlternativeTitle(String ddId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(d:DesignDecision) where d.id=$id match (d)-[a:ausgewaehlt]->(c:DDAlternative) return c.name", parameters("id", ddId));
            if (result != null) {
    ...
    public String getDesignDecisionTitle(String ddId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(d:DesignDecision) where d.id=$id return d.title", parameters("id", ddId));
            if (result != null) {
    ...
    public List<String> getRationaleIdForArcId(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision)-[c:wirdBegründetDurch]->(z:Rationale) return z.id", parameters("id", arcId));
            if (result != null) {
    ...
    public String getRationaleBeschreibung(Integer arcId, String rationaleId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) match(z:Rationale) where Id(a)=$id and z.id=$rationaleId match (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision)-[c:wirdBegründetDurch]->(z) return z.beschreibung", parameters("id", arcId, "rationaleId", rationaleId));
            if (result != null) {
    ...
    public String getRationaleReferenz(Integer arcId, String rationaleId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) match(z:Rationale) where Id(a)=$id and z.id=$rationaleId match (a)-[r:bausteinsicht]->(s:Image:Baustein)-[d:hatDecision]->(v:DesignDecision)-[c:wirdBegründetDurch]->(z) return z.referenz", parameters("id", arcId, "rationaleId", rationaleId));
            if (result != null) {
    ...
    public int countQualitaetSzenarien(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hasQualityScenario]->(s:Qualitaetsscenario) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public int countRisiken(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatRisiken]->(s:Risiken) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getRisiken(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatRisiken]->(s:Risiken) return s.risiken", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public int countGlossar(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatGlossar]->(s:Glossar) return count(s)", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
                return single.get("count(s)").asInt();
            }
    ...
    public String getGlossar(Integer arcId) {
        Session session = this.driver.session();
        return session.writeTransaction(transaction -> {
            Result result = transaction.run("match(a:Arc42) where Id(a)=$id match (a)-[r:hatGlossar]->(s:Glossar) return s.glossar", parameters("id", arcId));
            if (result != null) {
                Record single = result.single();
    ...
    public void deleteAllGitHubSecurityAdvisory() {
        ...
        try (Session session = getDriver().session()) {
            session.writeTransaction(transaction -> {
                Result result = transaction.run("match(s:SecurityAdvisory) detach delete s return count(*)");
                if (result != null) {
                    Record record = result.single();
    ...
    public void deleteAnalyseResult(Integer arcId) {
        try (Session session = getDriver().session()) {
            if (arcId != null) {
                session.writeTransaction(transaction -> {
                    Result result = transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    " match (a)-[r:analyseergebnisse]->(s:AnalyseErgebnis) delete r,s " + System.lineSeparator() +
                                    "return count(s)",
                            parameters("id", arcId));
                    Integer count = null;
                    if (result != null) {
    ...
    public void saveAnalseResult(Integer arcId, String analyseResult) {
        try (Session session = getDriver().session()) {
            if (arcId != null) {
                session.writeTransaction(transaction -> {
                    Result result = transaction.run("match(a:Arc42) where Id(a)=$id" + System.lineSeparator() +
                                    " merge (a)-[r:analyseergebnisse]->(s:AnalyseErgebnis {result: $result}) " + System.lineSeparator() +
                                    "return count(s)",
            ...
            if (id != null) {
                return session.writeTransaction(transaction -> {
                    ...
                    if (aElementsResult != null) {
                        return aElementsResult.list(record -> record.get("a.name").asString().trim()).stream().reduce("", (partialString, element) -> partialString + ", " + element);
                    }
                    return "Keine";
                });
            }
